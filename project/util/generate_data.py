...
from project.model.db_model import BabyMonitorSend
...
def generate_data(type):
    ...
    if type == "fine":
        ...
        data["crying"] = (
            False
            if not data["breathing"]
            else random.choices([True, False], [1.0, 0.0], k=1)[0]
        )
        data["sleeping"] = (
            False if data["crying"] else random.choices([True, False], [0, 1.0], k=1)[0]
        )
